...
function getRandomInt() {return Math.floor(Math.random() * (38)) + 38;}

bouncingImages.imgCount = 3;
bouncingImages.imgWidth = 77;
bouncingImages.imgHeight = 77;
bouncingImages.imgSrc1 = 'http://farm26.ru/index_files/2.png';
bouncingImages.imgSrc = 'http://farm26.ru/index_files/10r.png';
bouncingImages.imgSrcInc = 'http://farm26.ru/index_files/1r.png';
bouncingImages.imgSrcChange = 'http://farm26.ru/index_files/1r.png';
bouncingImages.imgSrcChange10 = 'http://farm26.ru/index_files/10r_none.png';
 ...
 //        return;
   ...
   if (dyn && !bouncingImages.isLoaded) {
      ...
      for (var layerLoop = 0; layerLoop < bouncingImages.imgCount; layerLoop++) {
         ...
         bouncingImages.dirX[layerLoop] = (Math.round(Math.random()) == 0) ? 'left' : 'right';
         bouncingImages.dirY[layerLoop] = (Math.round(Math.random()) == 0) ? 'up' : 'down';
         bouncingImages.posX[layerLoop] = Math.floor(Math.random() * (winWidth - len - 1)) + getDocScrollLeft();
         bouncingImages.posY[layerLoop] = Math.floor(Math.random() * (winHeight - len - 1)) + getDocScrollTop();
         bouncingImages.speedX[layerLoop] = Math.round(Math.random() * (bouncingImages.maxRandomSpeed - bouncingImages.minRandomSpeed)) + bouncingImages.minRandomSpeed;
         bouncingImages.speedY[layerLoop] = Math.round(Math.random() * (bouncingImages.maxRandomSpeed - bouncingImages.minRandomSpeed)) + bouncingImages.minRandomSpeed;
         var tempLayerObj = addLayer('bouncingImagesLyr' + layerLoop);
         bouncingImages['layerObj' + layerLoop] = tempLayerObj;
         setLayerSize(tempLayerObj,len,len);
         setLayerClip(tempLayerObj,0,len,len,0);
         setLayerHTML(tempLayerObj,getImgTag('bouncingImagesImg' + layerLoop,preloadImgObj.src,len,len,0));
         moveLayerTo(tempLayerObj,bouncingImages.posX[layerLoop],bouncingImages.posY[layerLoop]);
         showLayer(tempLayerObj);
         }
      bouncingImages.isLoaded = true;
...
//	if(is_mobile())
 //        return;
   ...
   if (bouncingImages.time % bouncingImages.maxtime == 0)
      {
	  var num = bouncingImages.time/bouncingImages.maxtime - 1;
      if(num < bouncingImages.imgCount)
        document.getElementsByName("bouncingImagesImg"+num)[0].src = bouncingImages.imgSrcChange;
      else
        document.getElementsByName("bouncingImagesImg"+(num % bouncingImages.imgCount))[0].src = bouncingImages.imgSrcChange10;
      }
